...
if (mouse_check_button_pressed(mb_left)) {
    if (point_in_box(_mx, _my, window_x1, window_y1, window_x2, window_y1 + 32)) {
        is_dragging = true; drag_dx = window_x1 - _mx; drag_dy = window_y1 - _my;
    }
}
...
if (is_dragging) {
    window_x1 = _mx + drag_dx; window_y1 = _my + drag_dy;
    ...
    btn_main_menu = [[_btn_base_x, _btn_base_y, _btn_base_x + _btn_w, _btn_base_y + _btn_h, "FIGHT"],
...
switch (current_state) {
    ...
    case BATTLE_STATE.PLAYER_TURN:
        ...
        if (current_menu == MENU.MAIN || current_menu == MENU.FIGHT) {
            if (_up) menu_focus = max(0, menu_focus - 2); if (_down) menu_focus = min(3, menu_focus + 2);
    ...
    case BATTLE_STATE.WIN_DOWNLOAD_PROGRESS:
        if (download_current_percent < download_end_percent) { download_current_percent += 0.25; } 
            ...
            download_current_percent = download_end_percent; current_state = BATTLE_STATE.WIN_DOWNLOAD_COMPLETE; alarm[0] = 120; 
            if (download_current_percent >= 100) {
                var _key = current_opponent_data.critter_keys[0]; var _data = global.bestiary[$ _key]; var _level = enemy_critter_data.level;
    ...
    case BATTLE_STATE.WIN_XP_GAIN:
        var _xp_gain = 100; player_critter_data.xp += _xp_gain; battle_log_text = player_critter_data.nickname + " gained " + string(_xp_gain) + " XP!";
    ...
    case BATTLE_STATE.WIN_END:
        ...
        if (mouse_check_button_pressed(mb_left) || keyboard_check_pressed(vk_enter)) {
            if (is_casual == false) { global.PlayerData.team[0] = player_critter_data; global.PlayerData.current_opponent_index++; } else { global.PlayerData.team[0] = player_critter_data; }
    ...
    case BATTLE_STATE.LOSE:
        ...
        if (is_casual == false && !global.bargain_offered) {
            if (current_opponent_data.name == "BronzeMod") { global.bargain_offered = true; array_push(global.unread_messages, { from: "???", message: "...you cannot win." }); }
        }
        if (mouse_check_button_pressed(mb_left) || keyboard_check_pressed(vk_enter)) {
            global.PlayerData.team[0] = player_critter_data; instance_destroy(player_actor); instance_destroy(enemy_actor); instance_destroy();
        }
        break;
